...
#define WG_SIZE (BLOCKS_IN_TILE_Y * BLOCKS_IN_TILE_X)
...
#ifndef INV_TILE_ORDER
#define lA(x,y) a_tile[(x)][(y)]
#define lB(x,y) b_tile[(x)][(y)]
#else
#define lA(x,y) a_tile[(y)][(x)]
#define lB(x,y) b_tile[(y)][(x)]
#endif
        ...
        __global float * restrict C,int ldc)
{
    ...
    #else
    ALIGN_FLOAT4 __local float a_tile[TILE_SIZE_M][TILE_SIZE_K+1];
    ALIGN_FLOAT4 __local float b_tile[TILE_SIZE_N][TILE_SIZE_K+1];
    #endif
    ...
    for(k=0;k<K;k+=TILE_SIZE_K) {
        ...
        for(int dk=0;dk<lmt;dk++) {
            ...
            for(int dr=0;dr<BLOCK_SIZE_M;dr++) {
                ap[dr] = lA(dk,block_row+dr);
            }
            for(int dc=0;dc<BLOCK_SIZE_N;dc++) {
                bp[dc] = lB(dk,block_col+dc);
            }
            for(int dr=0;dr<BLOCK_SIZE_M;dr++) {
                for(int dc=0;dc<BLOCK_SIZE_N;dc++) {
